...
async function request(path,options={}){const headers={'Content-Type':'application/json',...(options.headers||{})};const token=sessionStorage.getItem(tokenKey);if(token)headers.Authorization=`Bearer ${token}`;const r=await fetch(API+path,{...options,headers});const d=await r.json();if(r.status===401){window.dispatchEvent(new Event('auth:invalid'));throw Error(d.error||'Your session has expired.');}if(!r.ok)throw Error(d.error||'Something went wrong');return d}
