...
async function request(path,options={}){const headers={'Content-Type':'application/json',...(options.headers||{})};const token=sessionStorage.getItem(tokenKey);if(token)headers.Authorization=`Bearer ${token}`;try{const r=await fetch(API+path,{...options,headers});const contentType=r.headers.get('content-type')||'';const d=contentType.includes('application/json')?await r.json():null;const message=d?.error||(r.status===404?'The API endpoint was not found. Check the deployed VITE_API_URL.':`Request failed (${r.status}).`);if(r.status===401){window.dispatchEvent(new Event('auth:invalid'));throw Error(message||'Your session has expired.')}if(!r.ok)throw Error(message);return d}catch(error){if(error instanceof TypeError)throw Error('Unable to reach the API. Check the server connection and VITE_API_URL.');throw error}}
